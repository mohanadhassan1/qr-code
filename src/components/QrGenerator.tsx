...
type Mode = "redirect" | "website";

const DEFAULT_REDIRECT_URL = "https://www.runanddrive.ae/app-redirect";
...
export default function QrGenerator() {
  ...
  const [websiteUrl, setWebsiteUrl] = useState("https://www.runanddrive.ae");
  const [redirectUrl, setRedirectUrl] = useState(DEFAULT_REDIRECT_URL);

  const [size, setSize] = useState(900); // print-friendly
  const [logoRatio, setLogoRatio] = useState(0.20); // 20% of QR width
  const [margin, setMargin] = useState(2);

  const [error, setError] = useState<string | null>(null);

  const targetUrl = useMemo(() => {
    return mode === "redirect" ? redirectUrl.trim() : websiteUrl.trim();
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        // 2) Overlay logo + white background
    ...
    run();
  }, [targetUrl, size, logoRatio, margin, canGenerate]);
  ...
  const downloadPng = () => {
    ...
    a.download =
      mode === "redirect" ? "qr-redirect.png" : "qr-website.png";
    a.click();
  };
    ...
    <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
      <div className="rounded-2xl border bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Generate a QR for a redirect link or a website link — with your logo embedded.
        </p>
